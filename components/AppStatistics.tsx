...
export const AppStatistics: React.FC<AppStatisticsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'chats' | 'tokens' | 'models'>('overview');
  ...
  const handleExportReport = () => {
    const reportData = {
      title: "AI Studio Nexus Usage & Statistics Report",
    ...
    a.href = url;
    a.download = `usage-statistics-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const content = (
    <div className={`flex flex-col bg-[var(--sidebar-bg)] border border-[var(--border-color)] rounded-3xl w-full shadow-2xl overflow-hidden ${isEmbedded ? '' : 'max-w-4xl max-h-[90vh]'}`}>
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-color)] bg-[var(--sidebar-bg)]">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-[var(--text-primary)]">
              App Activity, AI Use & Usage Statistics
            </h2>
            <p className="text-xs text-[var(--text-secondary)]">
              Track active session time, tokens, Workspace API calls, and model metrics
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="text-[11px] font-semibold text-cyan-300 hover:text-white px-2.5 py-1 rounded-lg bg-cyan-950/60 hover:bg-cyan-900/80 border border-cyan-500/30 transition-all flex items-center gap-1"
      ...
      {/* Tab Navigation */}
      <div className="flex items-center px-5 border-b border-[var(--border-color)] bg-[var(--sidebar-bg)] gap-2 overflow-x-auto">
        {[
          { id: 'overview', label: '1. Overview & Time' },
          { id: 'chats', label: '2. Most Active Chats' },
          { id: 'tokens', label: '3. Token & Compute' },
          { id: 'models', label: '4. Per-Model Usage' }
        ].map(t => (
          <button
            key={t.id}
      ...
      {/* Scrollable Body */}
      <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6 custom-scrollbar max-h-[70vh]">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-5 animate-fade-in">
            ...
            {/* Multimodal, Workspace & Command Highlights */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="p-3.5 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] flex items-center justify-between">
                <div className="flex items-center space-x-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-[var(--text-primary)] truncate">Images Created</div>
                    <div className="text-[10px] text-[var(--text-secondary)]">Art & Studio</div>
                  </div>
                </div>
                <span className="text-sm font-bold font-mono text-purple-400">{stats.totalImagesCreated}</span>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] flex items-center justify-between">
                <div className="flex items-center space-x-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-[var(--text-primary)] truncate">Voice Transcripts</div>
                    <div className="text-[10px] text-[var(--text-secondary)]">Speech-to-text</div>
                  </div>
                </div>
                <span className="text-sm font-bold font-mono text-emerald-400">{stats.totalVoiceTranscripts}</span>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] flex items-center justify-between">
                <div className="flex items-center space-x-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-[var(--text-primary)] truncate">Workspace Calls</div>
                    <div className="text-[10px] text-[var(--text-secondary)]">Drive, Gmail, Docs</div>
                  </div>
                </div>
                <span className="text-sm font-bold font-mono text-blue-400">{stats.totalWorkspaceApiCalls || 0}</span>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] flex items-center justify-between">
                <div className="flex items-center space-x-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-[var(--text-primary)] truncate">App Commands</div>
                    <div className="text-[10px] text-[var(--text-secondary)]">Slash commands</div>
                  </div>
                </div>
                <span className="text-sm font-bold font-mono text-amber-400">{stats.totalAppCommandsExecuted || 0}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'models' && (
          <div className="space-y-4 animate-fade-in">
            <div className="text-xs text-[var(--text-secondary)]">
              Breakdown of token usage and message volume per virtual AI model
            </div>
            ...
            {(!stats.modelUsageBreakdown || Object.keys(stats.modelUsageBreakdown).length === 0) ? (
              <div className="text-center py-12 text-xs text-[var(--text-secondary)] bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)]">
                No per-model usage records yet. Send messages with different models to populate breakdown!
              </div>
            ) : (
              <div className="space-y-2">
                {Object.entries(stats.modelUsageBreakdown).map(([modelId, mData]) => (
                  <div
                    key={modelId}
                    className="p-3.5 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] flex items-center justify-between"
                  >
                    <div>
                    ...
                    <div className="flex items-center space-x-4 text-xs font-mono text-right">
                      <div>
                        ...
                        <div className="text-[9px] text-[var(--text-secondary)]">Prompt Tok</div>
                      </div>
                      <div>
                        <div className="text-purple-400 font-semibold">{mData.responseTokens.toLocaleString()}</div>
                        <div className="text-[9px] text-[var(--text-secondary)]">Resp Tok</div>
                      </div>
                    </div>
                  </div>
                ))}
